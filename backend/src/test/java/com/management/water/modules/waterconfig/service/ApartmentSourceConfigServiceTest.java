...
class ApartmentSourceConfigServiceTest {
  ...
  @Mock private ApartmentSourceConfigRepository repository;

  @Mock private ApartmentRepository apartmentRepository;

  @Mock private WaterSourceRepository sourceRepository;

  @InjectMocks private ApartmentSourceConfigService service;
  ...
  void shouldCreateConfigurationSuccessfully() {
    ...
    when(repository.existsByApartmentIdAndSourceId(1L, 1L)).thenReturn(false);

    when(repository.findByApartmentId(1L)).thenReturn(List.of());
  ...
  void shouldThrowExceptionWhenApartmentNotFound() {
    ...
    RuntimeException exception = assertThrows(RuntimeException.class, () -> service.create(config));
  ...
  void shouldThrowExceptionWhenSourceAlreadyExists() {
    ...
    when(repository.existsByApartmentIdAndSourceId(1L, 1L)).thenReturn(true);

    RuntimeException exception = assertThrows(RuntimeException.class, () -> service.create(config));

    assertEquals("Source already configured for this apartment", exception.getMessage());
  }
  ...
  void shouldThrowExceptionWhenRatioIsInvalid() {
    ...
    assertEquals("Invalid ratio value", exception.getMessage());
  }
  ...
  void shouldThrowExceptionWhenTotalRatioExceeds100() {
    ...
    when(repository.findByApartmentId(1L)).thenReturn(List.of(existingConfig));

    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> service.create(newConfig));

    assertEquals("Total ratio exceeds 100%", exception.getMessage());
  }
