...
public class ApartmentSourceConfigService {
  ...
  public ApartmentSourceConfig create(ApartmentSourceConfig config) {
    ...
    Apartment apartment =
        apartmentRepository
            .findById(config.getApartment().getId())
    ...
    boolean exists = repository.existsByApartmentIdAndSourceId(apartment.getId(), source.getId());
    ...
    List<ApartmentSourceConfig> existingConfigs = repository.findByApartmentId(apartment.getId());
